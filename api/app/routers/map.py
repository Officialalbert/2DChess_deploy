...
def move_player(payload: schemas.PositionUpdate, db: Session = Depends(get_db)):
    ...
    new_x = max(0, min(MAP_WIDTH, position.x + payload.dx))
    new_y = max(0, min(MAP_HEIGHT, position.y + payload.dy))
    position.x, position.y = new_x, new_y
    db.commit()
    db.refresh(position)

    # кэшируем актуальные позиции всех игроков в Redis, чтобы не бить БД на каждый poll
    redis_client.hset(
        "player_positions",
        str(payload.user_id),
